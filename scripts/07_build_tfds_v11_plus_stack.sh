...
LOG_FILE="${LOG_DIR}/v11_plus_stack_tfds_build.log"

mkdir -p "${LOG_DIR}"
exec > >(tee "${LOG_FILE}") 2>&1

ACTION_LABEL_SOURCE="${ACTION_LABEL_SOURCE:-command_delta}"
RACCOON_RLDS_INTERMEDIATE_ROOT="${RACCOON_RLDS_INTERMEDIATE_ROOT:-${PROJECT_ROOT}/Mujoco/raccoon_dataset/openvla_rlds_intermediate_v11_plus_stack_from_v11_close004_release_fk_${ACTION_LABEL_SOURCE}}"
if [[ "${RACCOON_RLDS_INTERMEDIATE_ROOT}" != /* ]]; then
  RACCOON_RLDS_INTERMEDIATE_ROOT="${PROJECT_ROOT}/${RACCOON_RLDS_INTERMEDIATE_ROOT}"
fi
export RACCOON_RLDS_INTERMEDIATE_ROOT
