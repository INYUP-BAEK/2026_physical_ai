...
LOG_FILE="${LOG_DIR}/v11_plus_stack_convert_rlds.log"

mkdir -p "${LOG_DIR}"
exec > >(tee "${LOG_FILE}") 2>&1

RAW_ROOT="${RAW_ROOT:-${PROJECT_ROOT}/Mujoco/raccoon_grasp_v11_plus_stack_raw}"
ACTION_LABEL_SOURCE="${ACTION_LABEL_SOURCE:-command_delta}"
OUT_ROOT="${OUT_ROOT:-${PROJECT_ROOT}/Mujoco/raccoon_dataset/openvla_rlds_intermediate_v11_plus_stack_from_v11_close004_release_fk_${ACTION_LABEL_SOURCE}}"
...
CLOSED_GRIPPER_MIN_XY_ACTION="${CLOSED_GRIPPER_MIN_XY_ACTION:-0.002}"
PROMOTE_PRE_CLOSE_STEPS="${PROMOTE_PRE_CLOSE_STEPS:-3}"
INITIAL_CLOSE_MIN_Z_ACTION="${INITIAL_CLOSE_MIN_Z_ACTION:-0.004}"
STACK_RELEASE_OPEN_REPEAT="${STACK_RELEASE_OPEN_REPEAT:-2}"
