...
def _default_intermediate_root() -> Path:
    project_root = Path(__file__).resolve().parents[3]
    return project_root / "Mujoco" / "raccoon_dataset" / "openvla_rlds_intermediate"


INTERMEDIATE_ROOT = Path(
    os.environ.get("RACCOON_RLDS_INTERMEDIATE_ROOT", _default_intermediate_root())
).expanduser().resolve()
...
class RaccoonPickPlace(tfds.core.GeneratorBasedBuilder):
                ...
                "steps": tfds.features.Dataset({
                    ...
                    "action": tfds.features.Tensor(
                        ...
                        doc="EEF_POS action: [dx,dy,dz,droll,dpitch,dyaw,gripper_cmd]. "
                            "For v12, dpitch may store pitch_alpha command for gripper orientation.",
                    ),
    ...
    @staticmethod
    def _build_command_ee_from_raw(step: dict) -> np.ndarray:
        joint_angles = step.get("joint_angles", [])
        if len(joint_angles) < 3:
            raise ValueError(f"Expected at least 3 joint angles for FK, got {len(joint_angles)}")
        ...
        z_cm = z
        return np.asarray([x_cm / 100.0, y_cm / 100.0, z_cm / 100.0], dtype=np.float32)

    @staticmethod
    def _build_action_from_raw(cur_step: dict, next_step: dict) -> np.ndarray:
        cur_ee = RaccoonPickPlace._build_command_ee_from_raw(cur_step)
        nxt_ee = RaccoonPickPlace._build_command_ee_from_raw(next_step)
        dpos = nxt_ee - cur_ee

        raw_action = cur_step.get("action", [0.0, 0.0, 0.0, 0.0])
        gripper_cmd = float(raw_action[3]) if len(raw_action) >= 4 else 0.0
        pitch_cmd = float(raw_action[4]) if len(raw_action) >= 5 else 0.0

        return np.array(
            [dpos[0], dpos[1], dpos[2], 0.0, pitch_cmd, 0.0, gripper_cmd],
            dtype=np.float32,
        )
