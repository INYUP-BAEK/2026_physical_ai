...
def _fingerprint_tfds_data_dir(data_dir: str) -> str:
    """Fingerprint local TFDS shards so dataset-statistics caches follow data rebuilds."""
    patterns = ("*.tfrecord*", "dataset_info.json", "features.json")
    paths = []
    for pattern in patterns:
        paths.extend(tf.io.gfile.glob(tf.io.gfile.join(str(data_dir), pattern)))

    parts = []
    for path in sorted(set(paths)):
        try:
            stat = tf.io.gfile.stat(path)
        except tf.errors.NotFoundError:
            continue
        mtime_nsec = getattr(stat, "mtime_nsec", 0)
        parts.append(f"{path}:{stat.length}:{mtime_nsec}")
    ...
    *,
    ...
    if isinstance(dataset_statistics, str):
        ...
        dataset_statistics = get_dataset_statistics(
            ...
            hash_dependencies=(
                str(builder.info),
                str(builder.data_dir),
                _fingerprint_tfds_data_dir(builder.data_dir),
                str(state_obs_keys),
